...
static bool TryFoldConstantDefault(const ParsedExpression &expr, const LogicalType &type, Value &result) {
	...
	if (inner.get().GetExpressionType() != ExpressionType::VALUE_CONSTANT) {
		return false;
	}
